...
## Activity Monitoring (Heartbeat v2)
...
of agents run concurrently without clobbering each other.

Per-ping (call whenever you use a tool or change state):

\`\`\`bash
node "${vaultPath.replace(/\\/g, '/')}/scripts/heartbeat.js" \\
  --vault="${vaultPath.replace(/\\/g, '/')}" \\
  --id="$CLAUDE_SESSION_ID" --name="Claude Code" --agent-type=claude \\
  --project="${opts?.project?.title || path.basename(projectDir)}" \\
...
\`\`\`bash
node "${vaultPath.replace(/\\/g, '/')}/scripts/heartbeat.js" --vault="${vaultPath.replace(/\\/g, '/')}" --id="$CLAUDE_SESSION_ID" --stop
\`\`\`
...
- Snapshots are written to \`.hypernovum/agents/<sessionId>.json\` (gitignored).
  The plugin polls the directory and renders one orb per session, colored by
  state, with an identity tooltip.
...
\`\`\`json
{
  "hooks": {
    "PreToolUse": [{
      "matcher": { "tool_name": ".*" },
      "hooks": [{ "type": "command",
        "command": "node \\"${vaultPath.replace(/\\/g, '/')}/scripts/heartbeat.js\\" --vault=\\"${vaultPath.replace(/\\/g, '/')}\\" --id=\\"$CLAUDE_SESSION_ID\\" --name=\\"Claude Code\\" --agent-type=claude --project=\\"${opts?.project?.title || path.basename(projectDir)}\\" --tool=\\"$TOOL_NAME\\"" }]
    }],
    "Stop": [{
      "hooks": [{ "type": "command",
        "command": "node \\"${vaultPath.replace(/\\/g, '/')}/scripts/heartbeat.js\\" --vault=\\"${vaultPath.replace(/\\/g, '/')}\\" --id=\\"$CLAUDE_SESSION_ID\\" --stop" }]
    }]
  }
}
\`\`\`
